...
namespace Jellyfin.Plugin.Allocine
{
    ...
    public sealed class AllocineRatingStore : IDisposable
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            try
            {
                await WriteMappingCoreAsync(request, allocineId, resolvedAt, cancellationToken).ConfigureAwait(false);
            }
            catch (SqliteException ex) when (IsCorruption(ex))
            {
                await RecoverFromCorruptionAsync(generation, ex, cancellationToken).ConfigureAwait(false);
            ...
            CancellationToken cancellationToken)
        {
            ...
            command.CommandText = """
                SELECT allocine_id, resolved_utc
                FROM mappings
            ...
            CancellationToken cancellationToken)
        {
            ...
            await using var command = connection.CreateCommand();
            command.CommandText = """
                INSERT INTO mappings (identity_key, media_type, imdb_id, tmdb_id, allocine_id, resolved_utc)
                VALUES ($identity_key, $media_type, $imdb_id, $tmdb_id, $allocine_id, $resolved_utc)
